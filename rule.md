...
- 자신에 대한 이의제기를 처리하는 것
    - 아래와 같은 경우에는 예외로 합니다.
        ...
        - 서버 주인이 업무를 처리하는 경우
        - 서버 주인의 허가가 있는 경우
    - 이 규정으로 인해 특정 운영진이 이의제기를 처리할 수 있지 아니한 경우, 다른 운영진 또는 서버 주인이 이의제기를 처리하여야 합니다. 다른 운영진이 처리하기 전까지는 이의제기는 처리되지 아니한 상태로 취급하며, 이의제기 대상이 된 업무 처리 건은 여전히 유효한 것으로 봅니다.
...
관리자 논의는 기본적으로 관리자 게시판에서 관리자, 서버 주인 간에 논의를 비공개적으로 진행합니다.
