...
7. ***채팅 분위기를 흐리는*** 행위. 이는 아래를 포함하지만 아레에 국한되지는 아니하며, 다음 중 하나 이상에 해당되더라도 분위기를 흐리지 아니한다고 운영진이 판단할 시 ‘금지 행위’로 정의하지 아니할 수 있습니다.
    ...
    - 유저 간 ***많은 소통이 이루어지는 채널에서 유저 간 갈등***에 대한 의견 표시 행위
        - 운영진이 갈등 중재 등 운영 업무 수행을 위해 운영진 본인의 의견을 표시하는 경우에는 이 조항에 해당되지 아니한 것으로 봅니다.
    - 유저 간 ***많은 소통이 이루어지는 채널에서 본인의 부정적인 감정을 표시***함으로서 서버 분위기에 악영향을 주는 행위
...
1. 서버 구성원에게 개인적으로 연락하거나 서버 내의 채널에 메시지를 보내는 등의 방법으로 ***특정 디스코드 서버나 기타 다른 홍보물을 홍보***하는 행위
    ...
    - 단, 마늘 서버와 "파트너 관계"에 해당되는 서버는 서버 주인이 별도로 명시할 경우 이 조항을 적용받지 아니합니다.
2. 제재를 받았는데 반성하지 않는 행위
3. ***불필요하게 티켓을 생성***하는 행위
...
6. 본인의 ***인맥 또는 지위를 악용***하거나 ***이용자들이 서버 운영에 대해 특정 입장을 가지도록 조장***하는 모든 행위
    - 여기서 “지위"란, 운영진 직책을 의미하지 아니하며 서버에 미치는 영향력 등을 의미합니다.
7. 운영진의 지시를 정당한 사유 없이 따르지 아니하거나 이에 대해 진지하지 아니한 태도로 응답하는 행위
8. ***서버 운영을 방해***하는 행위
9. 마늘 서버에서 타 서버로 병합되는 사항에 대하여 문의를 하는 행위
    - 이 조항에 의하여 제재하는 경우 최소 제재 수위는 경고 1개이며, 이 조항을 위반한 경우에는 별도로 소유자가 명시한 부분이 존재하지 않는 한 무조건 제재를 집행합니다.
    - 마늘 서버에서 타 서버로 병합되는 사항에 대한 문의만 불가하며, 타 서버에서 마늘 서버로 병합되는 사항에 대해서는 문의 가능합니다.
...
본 서버의 연합 및 파트너에 관한 사항에 대해서는 [별도의 규정](https://github.com/garlicfood1234/garlic-rule/blob/main/union_rule.md)에서 정의하는 바에 따릅니다.
