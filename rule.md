...
- 총관리자: 관리자를 대표하며 더 많은 업무를 처리합니다. 권한도 관리자보다 더 많이 부여받는 운영진입니다. 관리자가 권한이 부족하여 처리할 수 없는 업무를 처리할 수 있습니다. 또한 서버 주인이 장기간 부재중일 때 서버 주인의 권한을 일정 기간 대행받기도 합니다.
- 관리자: 서버의 특정 관리 업무를 맡아 수행하는 운영진입니다. 아래 3개 직책으로 구분되며 각 직책을 겸직할 수 있습니다.
    - 운영팀: 서버에서 규정을 위반한 사용자를 제재하고, 티켓을 처리합니다. 전반적인 서버의 채팅률에 기여합니다.
    - 적응팀: 서버에 새로 참여한 사용자가 서버에 적응하고 서버에서 신규 유저가 활동할 수 있도록 돕습니다.
    - 기획팀: 서버의 장기적인 운영 계획 및 이벤트 등을 계획합니다.

### 운영진 지원 절차

운영진에 지원하려는 사용자는 서버 주인에게 연락하여 운영진에 지원합니다. 단, 이 규정에 따라 운영진에 지원하기 위해 서버 주인에게 연락할 때는 서버 주인과 지원자만이 볼 수 있는 공간에서 문의해야 합니다.

### 운영진의 임명

운영진의 임명 방법은 아래 2가지 절차 중 하나로 합니다. 서버 주인의 판단에 따라 임명 절차를 간소화하거나 일부 또는 전체를 생략하여 임명하거나 이 규정에서 정의하지 않은 방법으로 임명할 수는 있으나, 운영진으로 임명되는 사용자가 운영진을 할 의사가 있음(직접 운영진에 지원했거나, 본인이 운영진으로 임명되는 것에 대해 긍정적인 입장을 표명하는 등)을 확인하는 절차는 생략할 수 없습니다.

- 서버 주인이 판단하기에 운영진으로 임명하기 적합한 사용자가 있는 경우 서버 주인이 해당 사용자에게 DM 등의 수단을 통해 개인적으로 연락하여 해당 사용자의 의사를 확인 후 임명할 수 있습니다.
- '운영진 지원 절차' 문단에 따라 특정 운영진 직책에 지원한 사용자(이하 "지원자")를 서버 주인의 판단에 따라 임명할 수 있습니다.
    - 서버 주인은 필요한 경우 지원자에게 질의응답을 진행할 수 있습니다.

### 운영진의 사퇴

- 운영진이 사퇴를 희망하는 경우 서버 주인에게 연락하여 사퇴 의사를 알리거나, 서버 내 공지 채널에 사퇴 의사를 표시하여야 합니다.
- 사퇴 의사를 표시한 이후부터는 운영진은 운영진 권한을 사용할 수 없습니다. 단, "긴급 사안" 또는 "경계 사안"에 해당되는 사안의 경우에는 예외로 합니다.

### 운영진의 해임

- 서버 주인은 아래와 같은 사유에 해당되는 경우에 운영진을 해임할 수 있습니다.
...
- 본인이 맡은 역할을 충실히 수행할 것
    - 본인이 맡은 역할이 아닌데 다른 직책에서 이미 해당 역할을 맡고 있는 경우 관여하지 아니하여야 합니다. 단, 긴급 사안의 경우 이 사항을 적용하지 아니합니다.
- 본 규정을 그 무엇보다도 우선할 것
...
- 일반 이용자에게 부여되지 않는 특수 권한이 있는 역할을 부여 또는 회수하는 것. 단, 아래와 같은 경우에는 예외로 합니다.
    - 긴급 사안인 경우. 이는 테러가 발생하여 마늘봇의 /권한회수 명령어로 특정 관리자의 권한을 회수하는 것을 포함합니다.
...
- `@everyone`이나 `@here` 멘션을 사용하거나 특정 역할을 멘션하는 것. 단, 아래와 같은 경우에는 예외로 합니다.
    ...
    - 서버 순인원이 (100의 배수)명 이상의 값에 도달하여 공지를 하는 경우
        - 단, 이는 서버 주인 또는 총관리자가 `@everyone`이나 `@here` 멘션을 사용하거나 특정 역할을 멘션하는 경우에만 적용됩니다.
    - 경계 사안인 경우. 단, 이 사항을 `@everyone`이나 `@here` 멘션을 사용하거나 특정 역할을 멘션하는 것이 허용되는 예외로 적용하려면 서버 주인 또는 총관리자의 동의가 필요합니다.
    - 서버 주인이 사전에 예외로 명시한 사안인 경우
    - 서버 주인이 업무를 처리하는 경우
    - 서버 주인의 허가가 있는 경우
...
관리자 논의는 기본적으로 관리자 게시판에서 관리자, 총관리자, 서버 주인 간에 논의를 비공개적으로 진행합니다. 이 때, 관리자의 하위 직책(운영팀, 기획팀, 적응팀)에 관계없이 모든 관리자가 논의에 참여할 수 있습니다.
...
- 임시 운영진의 담당 업무는 서버 주인이 지정한 업무에 한합니다. 단, "긴급 사안"으로 분류되는 업무 또는 "경계 사안"으로 분류되는 업무는 서버 주인이 지정한 업무가 아니더라도 처리할 수 있습니다.
...
- 임시 운영진은 필요에 따라 선출되며, 임기는 서버 주인이 결정하는 바에 따릅니다.
    - 임시 운영진을 임명하려는 경우 서버 주인은 공지 채널에 게시하여 지원자를 모집 후 서버 주인의 판단에 따라 선출하거나, "운영진의 임명" 문단에 명시된 바에 따라 임시 운영진을 임명합니다.
...
- 중요 사안: 서버 주인이 “중요 사안”으로 명시한 사안 또는 경계 사안에 해당되는 사안.
